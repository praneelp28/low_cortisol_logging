...
/* ── navigate without reload when possible ──────────────────── */
function applyUrl(newUrl){
  var status=document.getElementById('lcl-status');
  var currentTool=detectTool(location.href);

  if(currentTool==='kibana'){
    // Kibana: time is in hash. Changing hash doesn't reload.
    // Kibana watches hashchange and re-reads state.
    try{
      var nu=new URL(newUrl);
      window.location.hash=nu.hash;
      status.textContent='applied';
      status.style.color='#6ecf94';
      return;
    }catch(e){}
  }

  if(currentTool==='grafana'){
    // Grafana uses the `history` npm package which has its own listener
    // separate from native popstate. We need to find Grafana's internal
    // location service and call it directly.
    try{
      var nu=new URL(newUrl);
      var from=nu.searchParams.get('from');
      var to=nu.searchParams.get('to');

      // Strategy 1: Grafana's __grafanaRuntime (Grafana 10+)
      if(window.__grafanaRuntime&&window.__grafanaRuntime.locationService){
        window.__grafanaRuntime.locationService.partial({from:from,to:to});
        status.textContent='applied';
        status.style.color='#6ecf94';
        return;
      }

      // Strategy 2: Monkey-patch pushState to fire a custom event that
      // the history package picks up. Some Grafana builds listen for this.
      var origPush=history.pushState;
      history.pushState=function(){
        origPush.apply(history,arguments);
        window.dispatchEvent(new Event('pushstate'));
        window.dispatchEvent(new PopStateEvent('popstate',{state:arguments[0]}));
      };
      history.pushState({lcl:true},'',newUrl);
      history.pushState=origPush;

      // Give Grafana a moment to react
      setTimeout(function(){
        // Check if Grafana's time picker updated by looking at the URL
        // If Grafana picked it up, it may have re-written the URL itself
        var curFrom=new URL(location.href).searchParams.get('from');
        if(curFrom===from){
          status.textContent='applied';
          status.style.color='#6ecf94';
        }else{
          // Grafana didn't pick it up — fall back to reload
  ...
  // Thanos / fallback: full navigation (reload).
  // Using replace() so back button still works (doesn't add history entry).
  location.replace(newUrl);
}
...
document.getElementById('lcl-apply').onclick=function(){
  ...
  if(newUrl&&newUrl!==location.href){applyUrl(newUrl)}
